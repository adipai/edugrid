...
const TAAddNewContentBlock: React.FC = () => {
  ...
  const handleCreateBlock = async (action: string) => {
    try {
      ...
      if (action === "text") {
        navigate(
          `/ta/content-add-text?tb_id=${tb_id}&chap_id=${chap_id}&sec_id=${sec_id}&block_id=${blockId}`
        );
      } else if (action === "pic") {
        navigate(
          `/ta/content-add-pic?tb_id=${tb_id}&chap_id=${chap_id}&sec_id=${sec_id}&block_id=${blockId}`
        );
      } else if (action === "activity") {
        navigate(
          `/ta/content-add-activity?tb_id=${tb_id}&chap_id=${chap_id}&sec_id=${sec_id}&block_id=${blockId}`
        );
      }

      // navigate(`/ta/create-new-section?tb_id=${tb_id}&chap_id=${chapterId}`);
    } catch (error) {
...
export default TAAddNewContentBlock;
