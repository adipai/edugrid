...
const TAActivityAddQuestion: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/add_question",
        body
      );
      console.log("Block created:", response.data);
      // Redirect to the appropriate page based on the block type
      navigate(
        `/ta/content-add-activity?tb_id=${tb_id}&chap_id=${chap_id}&sec_id=${sec_id}&block_id=${block_id}&activity_id=${activity_id}`
      );
    } catch (error) {
...
export default TAActivityAddQuestion;
