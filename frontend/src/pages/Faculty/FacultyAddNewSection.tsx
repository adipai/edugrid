...
const FacultyAddNewSection: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/create_section",
        {
          tb_id: tb_id,
          chap_id: chap_id,
      ...
      navigate(
        `/faculty/create-new-block?tb_id=${tb_id}&chap_id=${chap_id}&sec_id=${sectionNumber}`
      );
    } catch (error) {
...
export default FacultyAddNewSection;
